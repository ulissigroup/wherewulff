...
from pymatgen.analysis.pourbaix_diagram import ELEMENTS_HO, PourbaixDiagram, PourbaixPlotter
...
composition_correction = {"V":-1.7, "Cr":-1.999, "Mn":-1.668, "Fe":-2.256, "Co":-1.638, "Ni":-2.541, "W":-4.438, "Mo":-3.202}
...
class BulkStabilityAnalysis(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        comp_dict_pbx = {str(key): value for key, value in bulk_composition.items if key not in ELEMENTS_HO}
        ...
        # Computed Entry
        computed_entry = ComputedEntry(composition=bulk_composition,
                                       energy=dft_energy,
        ...
        with MPRester() as mpr:
            ...
            phd_entry = PDEntry(bulk_composition,
        ...
        with MPRester() as mpr:
            ...
            pbx_entries, pbx_entry, eform_atom_pbx = self.get_pourbaix_entries(mpr, 
                                                                               computed_entry)

            pbx_entries.append(pbx_entry)

            # PBX Diagram
            pbx = PourbaixDiagram(pbx_entries, comp_dict=comp_dict_pbx, filter_solids=False)
        ...
        # Get pbx plot
        if pbx_plot:
            plt = PourbaixPlotter(pbx).plot_entry_stability(pbx_entry, label_domains=True)
            plt.savefig(f"{bulk_formula}_{magnetic_ordering}_pbx.png", dpi=300)

        
        # Export json file
        with open(f"{bulk_formula}_{magnetic_ordering}_stability_analys.json", "w") as f:
    ...
    def get_pourbaix_entries(self, mpr, comp_entry, solid_compat="MaterialsProject2020Compatibility"):
        """
        ...
        comp_dict = {str(key): value for key, value in entry_composition.items() if key not in ELEMENTS_HO}
        chemsys = list(comp_dict.keys())
        ...
        new_entry = ComputedEntry(comp_entry.composition, formation_energy, entry_id=comp_entry.entry_id)
        pbx_entry = PourbaixEntry(new_entry)
