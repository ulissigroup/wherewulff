...
class OER_SingleSite(object):
    ...
    def __init__(self, slab, adsorbates=oer_adsorbates_dict, random_state=42):
        ...
        # methods
        self.surface_coverage, self.ads_species, self.ads_indices, self.termination_info = self._get_surface_termination()
    ...
    def _get_surface_termination(self):
        ...
        termination_info = [[idx, site.specie, site.frac_coords]
    ...
    def _get_reference_slab(self):
        ...
        if self.surface_coverage[0] == "oxo":
            ...
            ads_indices_oxygen = [site[0] for site in self.termination_info if site[1] == Element("O")]
    ...
    def _mxidegen(self, repeat=[1,1,1], verbose=False):
        ...
        mxidegen = MXideAdsorbateGenerator(self.ref_slab, repeat=repeat, verbose=verbose)
        return mxidegen

    def _get_oer_intermediates(self, adsorbate, suffix=None, axis_rotation=[0,0,1], n_rotations=4):
        ...
        adsorbate_rotations = self.mxidegen.get_transformed_molecule(adsorbate, 
                                                                     axis=axis_rotation, 
                                                                     angles_list=adsorbate_angles)
        # Intermediates generation
        intermediates_dict = {}
        for ads_rot_idx in range(len(adsorbate_rotations)):
            ads_slab = self.ref_slab.copy()
            ads_slab = self._add_adsorbates(ads_slab, 
                                            self.bulk_like_site, 
                                            adsorbate_rotations[ads_rot_idx])
            if suffix:
                intermediates_dict.update({f"{adsorbate_label}_{suffix}_{ads_rot_idx}": ads_slab.as_dict()})
            else:
                intermediates_dict.update({f"{adsorbate_label}_{ads_rot_idx}": ads_slab.as_dict()})
    ...
    def generate_oer_intermediates(self, suffix=None):
        ...
        if self.surface_coverage[0] == "oxo":
            ...
            ooh_down = self._get_oer_intermediates(self.adsorbates["OOH_down"], suffix="down")
            oer_intermediates = {**reference_dict, **oh_intermediates, **ooh_up, **ooh_down}
            return oer_intermediates

        elif self.surface_coverage[0] == "oh":
            ox_intermediates = self._get_oer_intermediates(self.adsorbates["Ox"], n_rotations=1)
            ooh_up = self._get_oer_intermediates(self.adsorbates["OOH_up"], suffix="up")
            ooh_down = self._get_oer_intermediates(self.adsorbates["OOH_down"], suffix="down")
            oer_intermediates = {**reference_dict, **ox_intermediates, **ooh_up, **ooh_down}
            return oer_intermediates