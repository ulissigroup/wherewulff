...
class ForcesTrainer(BaseTrainer):
    ...
    def predict(
        ...
        if self.normalizers is not None and "target" in self.normalizers:
            ...
            #!mag, add magmom
            self.normalizers["magmom_target"].to(self.device)

        #!mag  predictions should refer to output??
        ...
        for i, batch_list in tqdm(
            ...
            if self.normalizers is not None and "target" in self.normalizers:
                ...
                #!mag
                out["magmom"] = self.normalizers["magmom_target"].denorm(
                    out["magmom"]
                )
    ...
    def _forward(self, batch_list):
        # forward pass.
        if self.config["model_attributes"].get("regress_forces", True):
            out_energy, out_mag, out_forces = self.model(batch_list)
        else:
            out_energy, out_mag = self.model(batch_list)

        if out_energy.shape[-1] == 1:
            out_energy = out_energy.view(-1)
            #!mag
            out_mag = out_mag.view(-1)
    ...
    def _compute_loss(self, out, batch_list) -> int:
        ...
        if self.config["task"].get("train_threshold_magmom", False):
            ...
            loss.append(
                    mag_mult
                    * self.loss_fn["energy"](
                        out["magmom"][mask_magmom], mag_target[mask_magmom]
                    )
                )
        else:
            loss.append(
...
#!如果采用mask 对mag小于0.1的不做处理，可能是这里？
    def _compute_metrics(self, out, batch_list, evaluator, metrics={}):
        ...
        if self.config["task"].get("eval_threshold_magmom", True):
            ...
            #!mag
            for natoms in target["natoms_mag"]:
                natoms_free_mag.append(
                #*calculate sum of values leading to no. of non-zero samples.
                    torch.sum(mask_magmom[s_idx_mag : s_idx_mag + natoms]).item()
                )
                s_idx_mag += natoms
        ...
        #!if normalize lables is False. not going throught three lines below
        if self.normalizer.get("normalize_labels", False):
        ...
        #!out excludes atoms masked. target only include atoms that fullfill threshold
        metrics = evaluator.eval(out, target, prev_metrics=metrics)
        return metrics

#!TODO modeify this when using run relaxations with magnetic ML models.
